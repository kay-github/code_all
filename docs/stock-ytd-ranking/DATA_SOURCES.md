...
1. 首次运行 GitHub Actions 全量 dry-run 与正式发布，核对 GitHub Hosted Runner 到 Baostock/Sina 的网络可达性和总耗时。
2. 建立 Worker 失败、SERVING_PREVIOUS、computed-fallback、日历覆盖不足和连续过期告警，并完成至少 10 个交易日影子对比。
3. 保存每批官方证券清单哈希、分交易所数量和源端点版本，支持股票池漂移审计。
4. 完成各公开数据源商用授权、再分发边界和调用频率确认。
5. 评估增量历史缓存；当前日终重新读取 YTD 端点优先保证复权修订可被重新计算。
