...
function attachTushareOperation(error, apiName) {
  if (!(error instanceof StockSourceError)) return error;
  const details = error.details && typeof error.details === "object"
    ? error.details
    : {};
  const providerMessage = String(details.message || "");
  const rateMatch = providerMessage.match(/每分钟\D{0,20}(\d+)\D{0,20}次/i);
  error.details = {
    ...details,
    apiName,
    rateLimitPerMinute: rateMatch ? Number(rateMatch[1]) : null
  };
  return error;
}
  ...
  const payload = await requestJson(options.baseUrl || "https://api.tushare.pro", {
    ...
    validateJson: (value) => {
      try {
        ...
        throw attachTushareOperation(error, apiName);
      }
    }
  });
