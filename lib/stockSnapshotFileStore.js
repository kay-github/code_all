...
async function loadCurrentEnvelope(directory) {
  ...
  try {
    return parseCurrentEnvelope(body);
  } catch (error) {
    throw storeError(
...
async function publishSnapshot(directory, snapshot, options = {}) {
  const { snapshotId, publishedSnapshot, envelope } =
    preparePublishedEnvelope(snapshot, options);
...
function createStockSnapshotFileStore(directory) {
  const root = rootDirectory(directory);
  return {
    type: "file",
    loadCurrentEnvelope: () => loadCurrentEnvelope(root),
    publishSnapshot: (snapshot, options) => publishSnapshot(root, snapshot, options),
    markServingPrevious: (expectedAsOf, errorCodes) =>
      markServingPrevious(root, expectedAsOf, errorCodes),
    withRefreshLock: (worker, options) => withRefreshLock(root, worker, options)
  };
}

module.exports = {
  createStockSnapshotFileStore,
  resolveInside,
  writeJsonAtomic,
  loadCurrentEnvelope,
  publishSnapshot,
