...
function buildComputedRecords(rows, dates) {
  ...
  for (const row of rows) {
    ...
    // 东财对刚上市新股常返回空 f26；无上市日期即无法证明存在年初基准，
    // 统一按 NEW_LISTING 排除（与年内新股同一口径），避免整批质量告警。
    const isNewListing = !listingDate || listingDate > dates.baseDate;
...
async function buildEmSnapshot(options = {}) {
  ...
  // 结算闸门：东财 f25 在盘中是实时值。f124 时间戳的主流日期必须等于目标
  // 交易日（即已收盘且过 18:30 截止），否则会把盘中数据当收盘快照发布。
  const freshRows = rows.filter((row) => row && row.sourceAsOf === dates.expectedAsOf).length;
  const freshnessRatio = rows.length === 0 ? 0 : freshRows / rows.length;
  if (freshnessRatio < 0.9) {
    throw workerError(
      "MARKET_DATA_NOT_SETTLED",
      "eastmoney quotes do not match the expected trading day (intraday or stale data)",
      {
        expectedAsOf: dates.expectedAsOf,
        freshRows,
        totalRows: rows.length,
        freshnessRatio: Math.round(freshnessRatio * 10000) / 10000
      }
    );
  }
