...
function snapshotWarning(snapshot, envelope = null) {
  ...
  if (snapshot.periodResetRequired) {
    ...
    warnings.push("参考行情源暂不可用，当前结果来自完整的独立复权计算快照。");
  } else if (
...
function validatePublishedSnapshot(snapshot, env = process.env) {
  try {
    ...
    if (
      !["adjusted-ytd.v1", "adjusted-ytd.v2"].includes(snapshot.methodologyVersion)
    ) {
      throw new Error("unsupported stock snapshot methodology");
    }
    if (
      snapshot.poolVersion !== (
        partitionedSourcePolicy ? "official-a-share.v2" : "a-share.v1"
      )
    ) {
    ...
    const expectedActiveSources = partitionedSourcePolicy
      ? ["baostock", "sina"]
      : ["tushare"];
    if (
      !sourceAudit ||
    ...
    if (
      ...
      (
        ...
        !["validated", "partially-validated", "computed-fallback"].includes(snapshot.sourceMode)
      )
    ) {
