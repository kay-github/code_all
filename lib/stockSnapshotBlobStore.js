...
// Blob 网关可能返回弱 ETag（W/"..."），而条件写 ifMatch 只匹配强格式；
// 统一在读取侧归一化，否则每次 current.json 条件更新都会 412。
...
function createStockSnapshotBlobStore(options = {}) {
  ...
  async function readObject(pathname, readOptions = {}) {
    ...
    return {
      body,
      etag: normalizeEtag(result.blob && result.blob.etag),
  ...
  // 区间统计等只读消费者按日期读取历史已发布快照；
  // 与 promoteLatestSnapshot 相同的候选选择规则，但不触碰 current。
  async function loadLatestSnapshotForDate(asOf) {
    const normalizedAsOf = normalizeDate(asOf, "asOf");
    const candidates = await listSnapshotsForDate(normalizedAsOf);
    for (const candidate of candidates) {
      const object = await readObject(candidate.pathname, {
        allowMissing: true,
        errorCode: "STOCK_SNAPSHOT_HISTORY_READ_FAILED",
        errorMessage: "failed to read the historical stock snapshot Blob"
      });
      if (!object) continue;
      let snapshot;
      try {
      ...
      if (
        ...
        snapshot.productionPublishable !== true ||
        snapshot.asOf !== normalizedAsOf ||
        !snapshot.stocks ||
        !Array.isArray(snapshot.records) ||
        candidate.pathname !== `${prefix}/snapshots/${snapshot.snapshotId}.json`
      ) {
        continue;
      }
      return {
  ...
  async function listAvailableSnapshotDates() {
    if (typeof client.list !== "function") {
      throw snapshotStoreError(
        "STOCK_SNAPSHOT_HISTORY_LIST_FAILED",
        "stock snapshot Blob listing is unavailable"
      );
    }
    const namePrefix = `${prefix}/snapshots/stock-ytd-`;
    const dates = new Set();
    const seenCursors = new Set();
    let cursor;
    do {
      let result;
      try {
        result = await client.list({ prefix: namePrefix, cursor, limit: 1000 });
      } catch (error) {
        throw snapshotStoreError(
          "STOCK_SNAPSHOT_HISTORY_LIST_FAILED",
          "failed to list historical stock snapshot Blobs",
          error
        );
      }
      if (!result || !Array.isArray(result.blobs)) {
      ...
      for (const blob of result.blobs) {
        if (!blob || typeof blob.pathname !== "string") continue;
        const match = blob.pathname
          .slice(`${prefix}/snapshots/`.length)
          .match(/^stock-ytd-(\d{4})(\d{2})(\d{2})-[a-f0-9]{16}\.json$/);
  ...
  // 区间统计 Phase 2：逐日 YTD 回填文件（backfill-qfq），
  // 写入需发布鉴权（api/stock-publish），读取供查询层作基准日数据源。
  function intervalDailyPath(asOf) {
    return `${prefix}/interval/daily/${asOf}.json`;
  }

  async function putIntervalDailyMap(asOf, payload) {
    const normalizedAsOf = normalizeDate(asOf, "asOf");
    // 因子修订后需要重灌，允许覆盖；路径由鉴权入口保护。
    try {
      await client.put(
        intervalDailyPath(normalizedAsOf),
        JSON.stringify(payload),
        jsonPutOptions({ allowOverwrite: true })
      );
    } catch (error) {
      throw snapshotStoreError(
        "STOCK_INTERVAL_DAILY_WRITE_FAILED",
        "failed to write the interval daily ytd Blob",
        error
      );
    }
  }

  async function loadIntervalDailyMap(asOf) {
    const normalizedAsOf = normalizeDate(asOf, "asOf");
    const object = await readObject(intervalDailyPath(normalizedAsOf), {
      allowMissing: true,
      errorCode: "STOCK_INTERVAL_DAILY_READ_FAILED",
      errorMessage: "failed to read the interval daily ytd Blob"
    });
    if (!object) return null;
    let payload;
    try {
      payload = JSON.parse(object.body);
    } catch (error) {
      return null;
    }
    if (
      ...
      payload.version !== "stock-ytd-interval-daily.v1" ||
      payload.asOf !== normalizedAsOf ||
      !payload.records ||
  ...
  // 逐日演变聚合文件：随日频文件灌入在鉴权入口增量维护（read-modify-write，
  // 上传方串行调用），供 ?series=1 查询直接返回，避免用户请求逐日读 Blob。
  function intervalSeriesPath() {
    return `${prefix}/interval/series.json`;
  }

  async function putIntervalSeries(payload) {
    try {
      await client.put(
        intervalSeriesPath(),
        JSON.stringify(payload),
      ...
      throw snapshotStoreError(
        "STOCK_INTERVAL_SERIES_WRITE_FAILED",
        "failed to write the interval series Blob",
        error
      );
    }
  }

  async function loadIntervalSeries() {
    const object = await readObject(intervalSeriesPath(), {
      allowMissing: true,
      errorCode: "STOCK_INTERVAL_SERIES_READ_FAILED",
      errorMessage: "failed to read the interval series Blob"
    });
    if (!object) return null;
    let payload;
    try {
    ...
    if (
      !payload ||
      typeof payload !== "object" ||
      payload.version !== "stock-ytd-interval-series.v1" ||
      !payload.days ||
  ...
  async function listIntervalDailyDates() {
    if (typeof client.list !== "function") {
      throw snapshotStoreError(
        "STOCK_INTERVAL_DAILY_LIST_FAILED",
        "stock snapshot Blob listing is unavailable"
      );
    }
    const namePrefix = `${prefix}/interval/daily/`;
    const dates = new Set();
    const seenCursors = new Set();
    let cursor;
    do {
      ...
      try {
        ...
        throw snapshotStoreError(
          "STOCK_INTERVAL_DAILY_LIST_FAILED",
          "failed to list interval daily ytd Blobs",
          error
        );
      }
      if (!result || !Array.isArray(result.blobs)) {
        throw snapshotStoreError(
          "STOCK_INTERVAL_DAILY_LIST_FAILED",
          "interval daily Blob listing returned an invalid response"
        );
      }
      for (const blob of result.blobs) {
        if (!blob || typeof blob.pathname !== "string") continue;
      ...
      if (!result.cursor || seenCursors.has(result.cursor)) {
        throw snapshotStoreError(
          "STOCK_INTERVAL_DAILY_LIST_FAILED",
          "interval daily Blob listing returned an invalid cursor"
        );
      }
      seenCursors.add(result.cursor);
      cursor = result.cursor;
  ...
  async function acquireLock(staleAfterMs) {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      try {
        ...
        return { ownerToken, etag: normalizeEtag(result.etag), metadata, failure: null };
      } catch (error) {
        if (!isPreconditionFailure(client, error)) {
          throw snapshotStoreError(
  ...
  async function heartbeatLock(lease) {
    ...
    try {
      ...
      lease.etag = normalizeEtag(result.etag);
  ...
  return {
    ...
    listIntervalDailyDates,
    loadIntervalDailyMap,
    loadIntervalSeries,
    loadLatestSnapshotForDate,
    putIntervalDailyMap,
    putIntervalSeries,
    markServingPrevious,
