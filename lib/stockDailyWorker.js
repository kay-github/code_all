...
const {
  ...
  publishSnapshot,
  withRefreshLock
} = require("./stockSnapshotFileStore");
...
async function runStockDailyWorker(options = {}) {
  ...
  return withRefreshLock(directory, async () => {
    try {
      ...
      let previous = null;
      try {
        previous = await loadCurrentEnvelope(directory);
      } catch (error) {
      ...
      if (
        ...
        if (previous.refreshStatus !== "PUBLISHED") {
          await publishSnapshot(directory, previous.snapshot, {
            expectedAsOf: dates.expectedAsOf,
      ...
      const envelope = await publishSnapshot(directory, candidate, {
        expectedAsOf: dates.expectedAsOf,
      ...
      try {
        const previous = await markServingPrevious(
          directory,
          expectedAsOf,
          [errorCode(error)]
        );
