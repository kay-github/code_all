...
// 与 lib/stockEmYtd.js boardForCode 同一规则；日频文件无 board 字段，从代码前缀推导。
function boardForRecord(code, exchange) {
...
const BOARD_ORDER = Object.freeze(["主板", "创业板", "科创板", "北交所"]);
...
function computeIntervalStats(baseMap, currentMap, options = {}) {
  ...
  // matched 已按 intervalReturn 升序，可直接取中位数；板块子集保持有序。
  const byBoard = [];
  for (const board of BOARD_ORDER) {
    const subset = matched.filter(
      (record) => boardForRecord(record.code, record.exchange) === board
    );
    if (!subset.length) continue;
    byBoard.push({
      board,
      matchedCount: subset.length,
      medianIntervalReturn: medianOfSorted(subset.map((record) => record.intervalReturn))
    });
  }
  ...
  return {
    ...
    medianIntervalReturn: medianOfSorted(matched.map((record) => record.intervalReturn)),
    byBoard,
    suspendedCount,
    excluded: {
...
// 逐日演变聚合：从一份日频文件（records: symbol → {exchange, ytd}）
// 算出该交易日的市场宽度切面（年初为基准）。分两个池存储：
// hs = 沪深，all = 沪深 + 北交所；中位数无法跨池合成，故两池都存。
function summarizeIntervalDay(records) {
  const hsValues = [];
  const allValues = [];
  for (const symbol of Object.keys(records)) {
    const record = records[symbol];
    if (!record || !Number.isFinite(record.ytd)) continue;
    const exchange = normalizeExchange(record.exchange);
    if (exchange === "SH" || exchange === "SZ") {
      hsValues.push(record.ytd);
      allValues.push(record.ytd);
    } else if (exchange === "BSE") {
      allValues.push(record.ytd);
    }
  }
  const summarizePool = (values) => {
    values.sort((left, right) => left - right);
    return {
      count: values.length,
      median: medianOfSorted(values),
      declines: DECLINE_THRESHOLDS_PCT.map((thresholdPct) =>
        values.reduce(
          (total, value) => total + (value < -thresholdPct / 100 - THRESHOLD_EPSILON ? 1 : 0),
          0
        )),
      gains: GAIN_THRESHOLDS_PCT.map((thresholdPct) =>
        values.reduce(
          (total, value) => total + (value > thresholdPct / 100 + THRESHOLD_EPSILON ? 1 : 0),
          0
        ))
  ...
  return { hs: summarizePool(hsValues), all: summarizePool(allValues) };
}
