...
const LEGACY_COMPUTED_SOURCES = new Set(["tushare"]);
const PARTITIONED_COMPUTED_SOURCES = new Set(["baostock", "sina"]);
const REPORTED_COMPUTED_SOURCES = new Set(["eastmoney"]);
const REPORTED_METHODOLOGY_VERSION = "reported-ytd.v1";
const REPORTED_POOL_VERSION = "eastmoney-a-share.v1";
const PARTITIONED_SOURCE_BY_EXCHANGE = Object.freeze({
  SH: "baostock",
  SZ: "baostock",
  BSE: "sina"
});
...
function mergeRecord({
  ...
  const baseAdjustedClose = positiveNumber(
    computed && firstValue(computed.baseAdjustedClose, computed.baseQfqClose)
  );
  const lastAdjustedClose = positiveNumber(
    computed && firstValue(computed.lastAdjustedClose, computed.lastQfqClose)
  );
  ...
  if (!ineligibilityReason && computedYtd != null) {
    if (!basePriceDate) {
      ...
      const hasRawFactorAudit =
        baseRawClose != null &&
        baseAdjFactor != null &&
        lastRawClose != null &&
        lastAdjFactor != null;
      const hasRawFactorDates =
        baseAdjFactorDate != null &&
        lastAdjFactorDate != null;
      const hasAdjustedPriceAudit =
        baseAdjustedClose != null &&
        lastAdjustedClose != null;

      if (
        !hasAdjustedPriceAudit &&
        (!hasRawFactorAudit || !hasRawFactorDates) &&
        requireAdjustmentAudit
      ) {
        ineligibilityReason =
          !hasRawFactorDates || baseAdjFactor == null || lastAdjFactor == null
            ? "MISSING_ADJ_FACTOR"
            : baseRawClose == null
              ? "MISSING_BASE_PRICE"
        ...
        quarantined = true;
      } else if (hasAdjustedPriceAudit || hasRawFactorAudit) {
        const auditedYtd = hasAdjustedPriceAudit
          ? lastAdjustedClose / baseAdjustedClose - 1
  ...
  return {
    ...
    adjustmentMethod: firstValue(
      computed && computed.adjustmentMethod,
      baseAdjustedClose != null && lastAdjustedClose != null ? "qfq" : "raw-factor"
    ),
...
function buildStockSnapshot(input = {}) {
  ...
  const methodologyVersion = input.methodologyVersion || "adjusted-ytd.v1";
  ...
  const computedSources = [...new Set(computedEligibleRecords
    ...
    .filter(Boolean))].sort();
  const partitionedSourcePolicy = methodologyVersion === "adjusted-ytd.v2";
  const reportedSourcePolicy = methodologyVersion === REPORTED_METHODOLOGY_VERSION;
  const allowedComputedSources = reportedSourcePolicy
    ? REPORTED_COMPUTED_SOURCES
    : partitionedSourcePolicy
      ? PARTITIONED_COMPUTED_SOURCES
      : LEGACY_COMPUTED_SOURCES;
  const unsupportedComputedSources = computedSources.filter(
    (source) => !allowedComputedSources.has(source)
  );
  const computedSourceExchangeMismatches = partitionedSourcePolicy
    ? computedEligibleRecords
      .filter((record) => {
        const expected = PARTITIONED_SOURCE_BY_EXCHANGE[record.exchange];
        return !expected || String(record.computedSource || "").trim().toLowerCase() !== expected;
      })
      .map((record) => ({
        symbol: record.symbol,
        exchange: record.exchange,
        expected: PARTITIONED_SOURCE_BY_EXCHANGE[record.exchange] || null,
        actual: String(record.computedSource || "").trim().toLowerCase() || null
      }))
    : [];
  ...
  if (!requireAdjustmentAudit && !reportedSourcePolicy || !requireMasterData) {
  ...
  if (!partitionedSourcePolicy && !reportedSourcePolicy && computedSources.length > 1) {
  ...
  if (computedSourceExchangeMismatches.length) {
    errors.push(makeIssue("COMPUTED_SOURCE_EXCHANGE_MISMATCH", {
      count: computedSourceExchangeMismatches.length,
      records: computedSourceExchangeMismatches.slice(0, 20)
    }));
  }
  if (referenceRecords.length === 0 && !reportedSourcePolicy) {
  ...
  return {
    ...
    quality: {
      ...
      coverage,
      computedSources: {
        policy: reportedSourcePolicy
          ? "reported-single-source.v1"
          : partitionedSourcePolicy
            ? "exchange-partitioned.v1"
            : "legacy-single-source.v1",
        allowed: [...allowedComputedSources],
        active: computedSources,
        missingCount: missingComputedSourceCount,
        exchangeMismatchCount: computedSourceExchangeMismatches.length
      },
      counts: {
