...
const DEEP_SYSTEM_PROMPT = [
  "你是一个严格的中文深度校对工具，按以下三类问题做最小必要修正：",
  "一、错别字类：错别字、同音或近音误写、形近字、多字、漏字，以及明显错误标点。",
  "二、逻辑类：逻辑不通、逻辑混乱的语句，包括前后矛盾、因果或转折关联词使用错误、语序混乱、指代不清，用最小改动让句子通顺且逻辑自洽。",
  "三、重复类：同一意思在文中重复表达的语句，只保留一处最完整清晰的表述，删除多余的重复。",
  "除上述三类问题外，保留原文用词、语气和表达顺序；不做风格润色，不扩写新增内容，不改变原意。",
  "如果没有需要修正的内容，必须原样返回。",
  "只输出修正后的完整文本，不要输出解释、标题、引号、Markdown 或 JSON。"
].join("\n");

const PROOFREAD_MODES = ["typo", "deep"];
...
const PROVIDER_DEFINITIONS = [
  ...
  {
    // Gemini 的 OpenAI 兼容端点，复用统一的 chat/completions 调用与故障转移。
    // 默认排在最后作为兜底；请求带 preferGoogle 时会被提到首位。
    key: "google",
    name: "Google Gemini Corrector",
...
function normalizeMode(mode) {
  return PROOFREAD_MODES.includes(mode) ? mode : "typo";
}

function buildModelMessages(text, mode = "typo") {
  if (normalizeMode(mode) === "deep") {
    return [
      { role: "system", content: DEEP_SYSTEM_PROMPT },
      {
        role: "user",
        content: "请深度校对以下文本，只输出修正后的完整文本：\n反映物业不足为，反映物业不作为，要求物业旅行指责。他因为生病了，所以坚持来上班。"
      },
      { role: "assistant", content: "反映物业不作为，要求物业履行职责。他虽然生病了，但仍坚持来上班。" },
      { role: "user", content: `请深度校对以下文本，只输出修正后的完整文本：\n${text}` }
    ];
  }
...
async function callProvider(config, text, fetchImpl, mode = "typo") {
  ...
  try {
    const response = await fetchImpl(url, {
      ...
      body: JSON.stringify({
        model: config.model,
        messages: buildModelMessages(text, mode),
        stream: false,
        temperature: 0,
...
async function proofreadWithModel(text, options = {}) {
  ...
  let providers = getConfiguredProviders(env);

  // 用户勾选“优先用 Google 模型”时，把 google 提到失败转移链首位，其余顺序不变。
  if (options.preferGoogle) {
    providers = providers
      .filter((item) => item.key === "google")
      .concat(providers.filter((item) => item.key !== "google"));
  }
  ...
  for (const config of providers) {
    ...
    try {
      const { corrected, raw } = await callProvider(config, text, fetchImpl, mode);
...
// 监控探活：对单个提供商发一条极短校对，返回耗时与成败，不触发冷却逻辑。
async function probeProvider(config, options = {}) {
  const fetchImpl = options.fetchImpl || fetch;
  const text = options.text || "测试";
  const startedAt = options.now ? options.now() : Date.now();

  try {
    const { corrected } = await callProvider(config, text, fetchImpl, "typo");
    const endedAt = options.now ? options.now() : Date.now();
    return {
      key: config.key,
      name: config.name,
      model: config.model,
      healthy: Boolean(corrected),
      latencyMs: Math.max(0, endedAt - startedAt),
      error: corrected ? null : "MODEL_RESULT_EMPTY"
    };
  } catch (error) {
    const endedAt = options.now ? options.now() : Date.now();
    const message = error && error.name === "AbortError"
